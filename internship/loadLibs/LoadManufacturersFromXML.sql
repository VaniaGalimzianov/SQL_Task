...
CREATE OR ALTER PROCEDURE LoadManufacturersFromXML
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Выполняем MERGE
        MERGE INTO dbo.Manufacturers AS target
        USING (
            SELECT 
            ...
            FROM @XmlData.nodes('/root/manufacturers/manufacturer') AS ItemData(Item)
        ) AS source
        ...
        WHEN NOT MATCHED THEN
            INSERT (id, name, created_date, last_update_date)
            VALUES (source.id, source.name, source.created_date, source.last_update_date);

        -- Логирование успешной загрузки
        SET @EndTime = SYSDATETIME();
        EXEC LogProcess 'LoadManufacturersFromXML', 'Successful data load into table Manufacturers', @StartTime, @EndTime;
    END TRY
    BEGIN CATCH
        SET @ErrorMessage = ERROR_MESSAGE();
        SET @LogError = 'Error: ' + @ErrorMessage;
        EXEC LogProcess 'LoadManufacturersFromXML', @LogError, @StartTime, @EndTime;
    END CATCH;
END;
